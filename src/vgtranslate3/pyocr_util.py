...
def image_to_boxes(image, lang=None, builder=None, mode=6):
    ...
    try:
        ...
        for lang_item in clang.split("+"):
            if (
                lang_item
                not in pyocr.libtesseract.tesseract_raw.get_available_languages(handle)
            ):
                raise pyocr.TesseractError(
                    "no lang", f"language {lang_item} is not available"
                )
        ...
        if res_iterator is None:
            raise pyocr.TesseractError("no script", "no script detected")
        page_iterator = (
            pyocr.libtesseract.tesseract_raw.result_iterator_get_page_iterator(
                res_iterator
            )
        )
        ...
        while True:
            ...
            confidence = (
                pyocr.libtesseract.tesseract_raw.result_iterator_get_confidence(
                    res_iterator, lvl_word
                )
            )
            ...
            if not pyocr.libtesseract.tesseract_raw.page_iterator_next(
                page_iterator, lvl_word
            ):
                break
