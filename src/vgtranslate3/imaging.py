...
def fill_fonts_wh():
    ...
    for i in range(len(FONTS)):
        ...
        for (
            char
        ) in "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя0123456789-+*=():&*^%$#@?!.,;'\"\u624b":
            t += 1
...
def drawTextBox(
    ...
    c = int(confid * 255)
    draw.rectangle(
        [x - 2, y, x + w + 2, y + h], fill=(0, 0, 0, 255), outline=(255, c, c, c)
    )
...
class ImageIterator:
    ...
    def _next_prev(cls, baseline=None, image_type=None, pre_next="prev"):
        ...
        if pre_next == "next" and baseline is not None:
            min_date = cls.date_order_convert(baseline)
        if pre_next == "prev" and baseline is not None:
            max_date = cls.date_order_convert(baseline)
        # just get the latest image
        for filename in iterator:
            if baseline:
                if min_date >= cls.date_order_convert(
                    filename
                ) or max_date <= cls.date_order_convert(filename):
                    continue
