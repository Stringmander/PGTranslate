...
from .util import get_color_counts_simple, reduce_to_multi_color

if os.name == "nt":
    from . import pyocr_util
else:
    import pytesseract


lang_to_tesseract_lang = {"deu": "deu", "eng": "eng", "jpn": "jpn", "rus": "rus"}
    ...
    if found_chars:
        for entry in found_chars:
            ...
            if curr_pos is None:
                ...
                if (
                    coords[0] > last_x - char_w
                    and curr_box[0] - char_w // 2
                    < coords[0]
                    < curr_box[2] + (char_w) * 3
                    and curr_box[0] - char_w // 2
                    < coords[2]
                    < curr_box[2] + (char_w) * 4
                    and curr_box[1] - char_h < coords[1] < curr_box[3] + (char_h)
                    and curr_box[1] - char_h < coords[3] < curr_box[3] + char_h
                ):
