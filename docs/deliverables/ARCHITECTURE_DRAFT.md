...
| --- | ---: | --- | --- | --- |
| `util.py` | 817 | | none | Largest module, ~35 free functions: base64 encode/decode, colour maths, bbox geometry, image compositing. No internal imports, so it is the leaf of the graph. |
...
| `serve.py` | 586 | | `config`, `imaging`, `ocr_providers`, `screen_translate`, `translation_providers`, `text_to_speech`, `util`, `bbox_extractor`, `webui.server` | Entry point. Also holds a second, threaded server path (`start_api_server`, :508) for a GUI window object that no longer exists in this codebase. |
...
- [ ] Was the local-model wiring ever present, or did the providers land without it? Check upstream history.
- [ ] Is the threaded `start_api_server` path reachable at all, or purely vestigial from the GUI original?
- [ ] Which provider does `local_server_api_key_type` override, and why does it coexist with `ocr_provider`?
- [ ] Does `ImageIterator`'s on-disk history write anything during normal server operation?
